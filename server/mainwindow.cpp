    ...
    , currentUser_("UNKNOWN")
{
    ...
    readUsersFromBd();              //Считываем пользователей из БД
    readMessagesFromBd();           //Считываем все сообщения из БД
    showUserForTable();             //Добавляем пользователей в таблицу
    showMessageToTable();           //Добавляем сообщения в таблицу
    runTcpServer();                 //Включаем сервер
}
...
void MainWindow::addToLog(const QString &str)
{
    ...
    QString m =  "<div style='color:black;'>" + QDateTime::currentDateTime().toString("[dd.MM.yyyy hh:mm:ss] ") + str + "</div><br>";
    ui->textLog->insertHtml(m);
    ui->textLog->moveCursor(QTextCursor::End);
}

//Вывести сообщение в поле журнала красным цветом
...
//Вывести сообщение в поле журнала зеленым цветом
...
{    
    stopTcpServer();                //Останавливаем сервер
...
//Добавить сообщения в таблицу БД
...
//Добавить строку в таблицу пользователей
...
//Проверить данные пользователя, полученные по сети
...
{
    bool b = false; //переменная для проверки
